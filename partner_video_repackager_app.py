...
def _reuters_access_token(
    ...
    """Create and cache a server-side Reuters OAuth token for under 23 hours."""
...
def _reuters_search_query(config: Dict[str, str], user_query: str) -> str:
    ...
    try:
        ...
        suggested = (
            interpreted.get("suggestedQuery")
            if isinstance(interpreted, dict)
            else None
        )
        query_clause = (
            suggested.get("queryClause") if isinstance(suggested, dict) else None
        )
        if query_clause:
...
def search_reuters_videos(
    ...
    return (
        items,
        f"Found {total_hits:,} entitled Reuters video(s); showing {len(items)}.",
    )
